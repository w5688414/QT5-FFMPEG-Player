...
void DisplayMediaTimer:: config(ImageFilter *mImageFilter,Media *mMedia,ReadPacketsThread* mReadPacketsThread){
    this->mMedia=mMedia;
    this->mImageFilter=mImageFilter;
    this->mReadPacketsThread=mReadPacketsThread;
}
...
void DisplayMediaTimer::synchronizeAudioAndVideo()
{
    ...
    if (mMedia->video->getVideoFrameSiez() == 0) {
    ...
    AVFrame * frame;
    frame = mMedia->video->dequeueFrame();

    // 将视频同步到音频上，计算下一帧的延迟时间
    double current_pts = *(double*)frame->opaque;
    double delay = current_pts - mMedia->video->getFrameLastPts();
    if (delay <= 0 || delay >= 1.0)
        delay = mMedia->video->getFrameLastDelay();
    mMedia->video->setFrameLastDelay(delay);
    mMedia->video->setFrameLastPts(current_pts);
    // 当前显示帧的PTS来计算显示下一帧的延迟
    double ref_clock = mMedia->audio->getAudioClock();
    ...
    mMedia->video->setFrameTimer(mMedia->video->getFrameTimer() + delay) ;

    double actual_delay = mMedia->video->getFrameTimer() - static_cast<double>(av_gettime()) / 1000000.0;
    if (actual_delay <= 0.010)
        actual_delay = 0.010;
    timer->setInterval(static_cast<int>(actual_delay * 1000 + 0.5));
    AVCodecContext *videoCtx = mMedia->video->getVideoStream()->codec;
    img = mImageFilter->filterColor(img);//色彩过滤
    uint8_t *data[AV_NUM_DATA_POINTERS] = { 0 };
    data[0] = (uint8_t *)img->bits();
    int linesize[AV_NUM_DATA_POINTERS] = { 0 };

    if (img->format() == QImage::Format_Grayscale8) {//灰度视频
        mMedia->video->swsContext = sws_getCachedContext(mMedia->video->swsContext, videoCtx->width,
            videoCtx->height,
            videoCtx->pix_fmt,
    ...
    int h = sws_scale(mMedia->video->swsContext, frame->data, frame->linesize, 0, videoCtx->height,
        data,
        linesize
    );//缩放图片
    img->setColorCount(1);
    img = mImageFilter->filter(img);//视频操作过滤器
    emit updateFrame(img,mReadPacketsThread);//发送信号到ui线程播放视频
